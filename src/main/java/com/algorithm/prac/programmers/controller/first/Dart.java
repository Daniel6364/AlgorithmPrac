    ...
    public static int solution(String dartResult) {
        ...
        for (int i = 0; i < dartResult.length(); i++) {
            ...
            if (dartResult.charAt(i) == 'S') {
                String val = String.valueOf(dartResult.charAt(i - 1));
                if (val.equals("0")) {
                    String ten = String.valueOf(dartResult.charAt(i - 2)) + val;

                    int returnA = Integer.parseInt(ten) * 1;
                    tmpStack.add(returnA);
                } else {
                ...
                if (val.equals("0")) {
                    String ten = String.valueOf(dartResult.charAt(i - 2)) + val;
                    int returnB = Integer.parseInt(ten) * Integer.parseInt(ten);
                    tmpStack.add(returnB);
                } else {
                    int b = Integer.parseInt(String.valueOf(val));
                    int returnB = b * b;
                    tmpStack.add(returnB);
                }
                ...
                if (val.equals("0")) {
                    String ten = String.valueOf(dartResult.charAt(i - 2)) + val;
                    int returnC = Integer.parseInt(ten) * Integer.parseInt(ten) * Integer.parseInt(ten);
                    tmpStack.add(returnC);
                } else {
                ...
                if (tmpStack.size() > 2) {
                    for (int j = tmpStack.size() - 1; j > 0; j--) {
                        tmpStack2.push(tmpStack.pop() * 2);
                    }

                    for (int val : tmpStack2) {
                        tmpStack.add(val);
                    }
                } else {
                    for (int tmpStackVal : tmpStack) {
                        tmpStack2.add(tmpStackVal * 2);
                    }
                    tmpStack = tmpStack2;
                }

            } else if (dartResult.charAt(i) == '#') {
                int tmpVal = tmpStack.peek() * -1;
                tmpStack.pop();
                tmpStack.add(tmpVal);
            }
