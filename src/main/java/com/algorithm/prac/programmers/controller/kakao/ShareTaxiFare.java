...
합승 택시 요금
        ...
        위 그림 예시에서, 4번 지점에서 1번 지점으로(4→1) 가거나, 1번 지점에서 4번 지점으로(1→4) 갈 때 예상 택시요금은 10원으로 동일하며
        이동 방향에 따라 달라지지 않습니다.
...
public class ShareTaxiFare {
    ...
    public int solution(int n, int s, int a, int b, int[][] fares) {
        ...
        for (int i = 1; i < n + 1; i++) {
            min = Math.min(min, node[s][i] + node[i][a] + node[i][b]); // 10 + 25 + 63
        }
    ...
    public int solution2(int n, int s, int a, int b, int[][] fares) {
        ...
        for (int i = 1; i < n + 1; i++) {
            ...
            for (int j = 1; j < n + 1; j++) {
                for (int k = 1; k < n + 1; k++) {
                    ...
                    if (i == j || costs[j][i] == 0 || costs[i][k] == 0) continue;
                    if (costs[j][k] == 0) {
        ...
        for (int i = 1; i < n + 1; i++) {
            if (costs[s][i] + costs[i][a] + costs[i][b] == 0) continue;
            answer = Math.min(costs[s][i] + costs[i][a] + costs[i][b], answer);
        }
    ...
    public static void main(String[] args) {
        ...
        ShareTaxiFare shareTaxiFare = new ShareTaxiFare();
//        System.out.println(shareTaxiFare.solution(n, s, a, b, fares));
        System.out.println(shareTaxiFare.solution2(n, s, a, b, fares));
