...
public class MenuRenewal2 {
    ...
    public static void reOrders(String order, int orderSize, int course, int start, String key) {

        if (course == 0) {
            hm.put(key, hm.getOrDefault(key, 0) + 1);
        } else {
            for (int i = start; i < order.length(); i++) {
//                if (!key.contains(String.valueOf(order.charAt(i)))) {
                    reOrders(order, orderSize, course - 1, start + 1, key + String.valueOf(order.charAt(i)));
//                }
            }
        }
    }
    ...
    public static String[] solution(String[] orders, int[] course) {
        ...
        for (String menu : orders) {
            ...
            String order = new String(charMenu);

            for (int cnt : course) {
                if (cnt <= order.length()) {
                    reOrders(order, order.length(), cnt, 0, "");
                }
            }
        ...
        for (int count : course) {
            ...
            for (String key : keyList) {
                if (1 < hm.get(key) && key.length() == count) {
                    if (hm.get(key) >= max_val) {
                        answerList.add(key);
                        max_val = hm.get(key);
                    }
                }
